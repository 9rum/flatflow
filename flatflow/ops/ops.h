...
// flatflow::OperatorRegistryBase
//
// A base class to ease sync up with types generated by the FlatBuffers
// compiler.
...
template <Operator>
internal::polynomial<OperatorRegistryBase::value_type> symbolic_trace_impl(
...
symbolic_trace_impl<Operator::_SOFTMAX>(
  ...
  auto expr = internal::polynomial<OperatorRegistryBase::value_type>(5);

  for (flatbuffers::uoffset_t index = 0; index < shape->size(); ++index) {
    CHECK_NE(shape->Get(index), nullptr);
    expr *= internal::polynomial<OperatorRegistryBase::value_type>(
...
symbolic_trace_impl<Operator::ADD_TENSOR>(
  ...
  auto expr =
      internal::polynomial<OperatorRegistryBase::value_type>(args->size());
...
symbolic_trace_impl<Operator::BMM>(
  ...
  const auto expr = internal::polynomial<OperatorRegistryBase::value_type>(
                        b->coef0(), b->coef1()) *
                    internal::polynomial<OperatorRegistryBase::value_type>(
                        n->coef0(), n->coef1()) *
                    internal::polynomial<OperatorRegistryBase::value_type>(
...
symbolic_trace_impl<Operator::MEAN_DIM>(
  ...
  auto expr = internal::polynomial<OperatorRegistryBase::value_type>(1);
...
symbolic_trace_impl<Operator::RSQRT>(
  ...
  auto expr = internal::polynomial<OperatorRegistryBase::value_type>(2);
...
symbolic_trace_impl<Operator::SILU>(
  ...
  auto expr = internal::polynomial<OperatorRegistryBase::value_type>(4);
...
decltype(auto) symbolic_trace(const Graph *graph) {
  ...
  auto expr = internal::polynomial<OperatorRegistry::value_type>();

  // clang-format off
  #pragma omp declare reduction(+ : flatflow::internal::polynomial<    \
          flatflow::OperatorRegistry::value_type> : omp_out += omp_in) \
  ...
  for (flatbuffers::uoffset_t index = 0; index < nodes->size(); ++index) {
    ...
    expr += registry.Dispatch(node->op(), node->args(), node->meta());
  }

  LOG(INFO) << absl::StrFormat("Traversing a graph with %u nodes took %fs", nodes->size(), omp_get_wtime() - now);
  // clang-format on

  const auto scale = std::gcd(std::gcd(expr[1], expr[2]), expr[3]);
  expr[0] = 0;
  expr /= scale;

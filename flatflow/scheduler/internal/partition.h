...
template <typename Value, typename Result>
  requires flatflow::arithmetic<Result>
class Subset {
 public:
  ...
  Subset(Result sum, const Value &item) : sum_(sum) {
    items_.emplace_back(item);
  }

  Subset(Result sum, Value &&item) : sum_(sum) {
    items_.emplace_back(std::move(item));
  }

  Subset(Result sum, const std::vector<Value> &items)
      : sum_(sum), items_(items) {}

  Subset(Result sum, std::vector<Value> &&items)
...
class Solution {
 public:
  ...
  Solution(InputIterator first, InputIterator last, Proj proj) {
    subsets_.reserve(std::distance(first, last));
    for (; first != last; ++first) {
      subsets_.emplace_back(std::invoke(proj, *first), std::move(*first));
    }
  ...
  std::vector<Subset<Value, Result>> &subsets() { return subsets_; }

  const std::vector<Subset<Value, Result>> &subsets() const { return subsets_; }
 ...
 protected:
  Result difference_;
  std::vector<Subset<Value, Result>> subsets_;
};
...
OutputIterator BLDM(InputIterator first, InputIterator last,
                    OutputIterator result, Proj proj,
  ...
  auto solutions = std::priority_queue<
      Solution<std::iter_value_t<InputIterator>,
               std::remove_cvref_t<std::invoke_result_t<
                   Proj, std::iter_value_t<InputIterator>>>>>();
...
// where projection `proj` is applied to the items. The resulting subsets are
// stored in an output range starting from `result`.
...
OutputIterator Partition(InputIterator first, InputIterator last,
                         ...
                         std::iter_difference_t<InputIterator> m,
                         Proj proj = {}) {
  return BLDM(first, last, result, proj, m);
}

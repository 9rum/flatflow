...
template <typename Index, typename Size, typename UnaryOp>
  requires(Unsigned<Index> && Unsigned<Size> &&
           Numerical<std::invoke_result_t<UnaryOp, Size>>)
class Subset {
 public:
  using container_type = std::vector<Index>;
  using result_type = typename std::invoke_result_t<UnaryOp, Size>;

  Subset(const std::pair<Size, Index> &item, UnaryOp op) {
    sum_ = op(item.first);
    items_ = {item.second};
  }
...
class Solution {
 public:
  using container_type = std::vector<Subset<Index, Size, UnaryOp>>;
  using result_type = typename container_type::value_type::result_type;

  Solution(std::span<const std::pair<Size, Index>> items, UnaryOp op) {
    subsets_.reserve(items.size());
    std::for_each(std::execution::seq, items.begin(), items.end(),
                  [&](const auto &item) { subsets_.emplace_back(item, op); });
...
template <typename Index, typename Size, typename UnaryOp>
  requires(Unsigned<Index> && Unsigned<Size> &&
           Numerical<std::invoke_result_t<UnaryOp, Size>>)
std::vector<Subset<Index, Size, UnaryOp>> BLDM(
  ...
  for (std::size_t offset = 0; offset < items.size(); offset += stride) {
    solutions.emplace(std::span(items).subspan(offset, stride), op);
  }

...
class Subset {
 public:
  ...
  Subset(key_type key, mapped_type &&item) {
    sum_ = key;
    items_ = {std::move(item)};
  }

  Subset(key_type key, const container_type &items) : items_(items) {
    sum_ = key;
  }
...
class Solution {
 public:
  using container_type =
      std::vector<Subset<std::invoke_result_t<Proj, std::iter_value_t<InputIt>>,
...
// PartitionPolicy
//
// A policy for selecting the algorithm upon partitioning.
...
// using the algorithm corresponding to the given `policy`, where each subset
// contains items transformed via `func`. The items are evaluated as their
// projected values through `proj` and the resulting subsets are stored in an
// output range starting from `result`.
template <typename InputIt, typename OutputIt, typename F, typename Proj>
OutputIt Partition(PartitionPolicy policy, InputIt first, InputIt last,
                   OutputIt result, F func, Proj proj,
                   typename std::iterator_traits<InputIt>::difference_type m) {
  switch (policy) {
    case PartitionPolicy::kBLDM:

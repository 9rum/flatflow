...
fn test_transform_with_llama3() -> Result<(), InvalidFlatbuffer> {
    ...
    let expr = transform(root_as_graph(builder.finished_data())?); // 8417 s0^2 + 663883653 s0
    assert_eq!(expr(0), Ok(0));
    assert_eq!(expr(1), Ok(663892070));
    assert_eq!(expr(1024), Ok(688642724864));
    assert_eq!(expr(2048), Ok(1394937178112));
